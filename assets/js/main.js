...
function renderProductCard(product, index = 0, options = {}) {
    ...
    return `
        <div class="product-card ${isBundle ? 'bundle-card' : ''}" 
             ...
             onclick="window.location.href='${window.QoffaRoute('product-detail')}?id=${product.id}'" 
    ...
    if (cart.length === 0) {
        sidebar.innerHTML = `
            <div class="cart-empty">
                ...
                <p>السلة فارغة</p>
                <a href="${window.QoffaRoute('products')}" class="btn btn-primary">تصفح المنتجات</a>
            </div>
        `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('cartCheckout')?.addEventListener('click', function() {
        ...
        window.location.href = window.QoffaRoute('order');
    });
