...
(() => {
  ...
  const getImage = product => product.product_image?.[0]?.url || 'assets/images/logo.png';
  ...
  const getCatalogue = () => {
    if (!cataloguePromise) {
      cataloguePromise = fetch(PRODUCTS_URL, { headers: { Authorization: `Token ${TOKEN}` } })
        ...
        .then(data => (data.results || []).filter(product => product.name).map(product => ({
